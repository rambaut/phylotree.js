...
(function() {
    ...
    d3.layout.phylotree = function(container) {
        ...
        phylotree.refresh = function() {
            ...
            nodes.attr("id", phylotree.id_node)
        ...
        phylotree.reclass_node = function(node) {
            ...
            if (d3_phylotree_is_leafnode(node)) {
                class_var += " " + css_classes["external-node"];
            }
        ...
        phylotree.id_node = function(node) {
            if (node.name) {
                return node.name;
            }
     ...
     *
     * @param {Node} node A node in a tree.
     * @returns {boolean} Whether or not the node is a leaf node.
     */
    ...
    /**
     * Determine if a given node is the root node.
     *
     * @param {Node} node node in a tree.
     * @returns {Bool} Whether or not the node is the root node.
     */
    function d3_phylotree_is_rootnode(node) {
        return (node.name && node.name == "root");
    }
